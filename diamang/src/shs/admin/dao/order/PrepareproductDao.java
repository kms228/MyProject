...
//		try {
//			con = DbcpBean.getConn();
//			String bodySql = "SELECT AAA.*, OL.ORDERCNT " + 
...
//							 	   		"FROM ORDERLIST O, ITEM I, BUYBOARD B, MEMBERS M " + 
//							 	   		"WHERE O.PNUM=I.PNUM AND O.BUY_NUM=B.BUY_NUM AND B.MNUM=M.MNUM AND STATE='상품준비중' ";
//			String desinenceSql = 	   " ORDER BY O.BUY_NUM) AA) AAA,(SELECT COUNT(*) ORDERCNT, BUY_NUM FROM ORDERLIST GROUP BY BUY_NUM) OL WHERE OL.BUY_NUM=AAA.BUY_NUM AND RNUM>=? AND RNUM<=?";
//			if(!vo.getItem_name().equals("")) {
...
//				PrepProdVo prepVo = new PrepProdVo(buy_num, order_num, amount, pnum, price, item_name, buy_date, accprice, name, ordercnt);
//				list.add(prepVo);				
//			}
//			return list;
//		} catch (SQLException e) {
...
//	}
	public ArrayList<PrepProdVo> search(SearchOptionVo vo, PagingVo pagingVo){
		...
		try {
			...
			bodySql = bodySql + "ORDER BY O.BUY_NUM DESC) AA) A3, " + 
					"(SELECT COUNT(*) ORDERCNT, BUY_NUM " + 
					"FROM (SELECT A1.ORDER_NUM, ROWNUM RNUM, A1.BUY_NUM " + 
					"FROM (SELECT B.BUY_NUM, O.ORDER_NUM " + 
					"FROM ORDERLIST O, ITEM I, BUYBOARD B, MEMBERS M " + 
					"WHERE O.PNUM=I.PNUM AND B.BUY_NUM=O.BUY_NUM AND B.MNUM=M.MNUM AND STATE='상품준비중' ";
			bodySql = bodySql + appendSql;
			String desinenceSql ="ORDER BY O.BUY_NUM DESC " + 
					") A1 " + 
					") " + 
					"WHERE RNUM>=? AND RNUM<=? " + 
					"GROUP BY BUY_NUM) A2 " + 
					"WHERE A2.BUY_NUM=A3.BUY_NUM AND RNUM>=? AND RNUM<=? " +
			...
			while(rs.next()) {
				...
				PrepProdVo prepVo = new PrepProdVo(buy_num, order_num, amount, pnum, price, item_name, buy_date, accprice, name, ordercnt,addr,caddr);
				list.add(prepVo);				
			}
			return list;
		} catch (SQLException e) {
