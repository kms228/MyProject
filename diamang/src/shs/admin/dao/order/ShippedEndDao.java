	...
	public int getCount(OrderSearchVo vo) {
		...
		try {
			...
			String sql = "SELECT NVL(COUNT(ORDER_NUM),0) CNT "
					...
					+ "WHERE O.PNUM=I.PNUM AND O.BUY_NUM=B.BUY_NUM AND B.MNUM=M.MNUM AND STATE='배송중'";			
	...
	public ArrayList<PrepProdVo> search(OrderSearchVo vo, PagingVo pagingVo){
		...
		try {
			...
			bodySql = bodySql + "ORDER BY O.BUY_NUM DESC) AA) A3, " + 
					...
					"FROM ORDERLIST O, ITEM I, BUYBOARD B, MEMBERS M " + 
					"WHERE O.PNUM=I.PNUM AND B.BUY_NUM=O.BUY_NUM AND B.MNUM=M.MNUM AND STATE='배송중' ";
			bodySql = bodySql + appendSql;
